...
export class ToastNotifier {
  ...
  async success(message: string, title?: string, duration?: number): Promise<void> {
    try {
      if (!this.client?.tui?.showToast) {
        console.warn('[opencode-models-discovery-wz] Toast API not available (client.tui.showToast missing)')
        return
      }
      ...
      console.error(`[opencode-models-discovery-wz] Failed to show success toast`, error)
    }
  }
  ...
  async error(message: string, title?: string, duration?: number): Promise<void> {
    try {
      ...
      console.error(`[opencode-models-discovery-wz] Failed to show error toast`, error)
    }
  }
  ...
  async warning(message: string, title?: string, duration?: number): Promise<void> {
    try {
      ...
      console.error(`[opencode-models-discovery-wz] Failed to show warning toast`, error)
    }
  }
  ...
  async info(message: string, title?: string, duration?: number): Promise<void> {
    try {
      ...
      console.error(`[opencode-models-discovery-wz] Failed to show info toast`, error)
    }
  }
  ...
  async progress(message: string, title?: string, progress?: number): Promise<void> {
    try {
      ...
      console.error(`[opencode-models-discovery-wz] Failed to show progress toast`, error)
    }
  }
  ...
  async detailed(options: {
    ...
    try {
      ...
      console.error(`[opencode-models-discovery-wz] Failed to show detailed toast`, error)
    }
  }
}
